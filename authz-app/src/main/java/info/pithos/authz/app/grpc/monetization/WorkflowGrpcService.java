...
public final class WorkflowGrpcService extends WorkflowServiceGrpc.WorkflowServiceImplBase {
    ...
    @Override public void get(GetByIdRequest req, StreamObserver<Workflow> obs)                  { GrpcSupport.respond(get.handle(req, GrpcSupport.context()), obs); }
    @Override public void listByApp(GetByIdRequest req, StreamObserver<WorkflowList> obs)        { GrpcSupport.respond(listByApp.handle(req, GrpcSupport.context()), obs); }
    @Override public void listByJourney(GetByIdRequest req, StreamObserver<WorkflowList> obs)    { GrpcSupport.respond(listByJourney.handle(req, GrpcSupport.context()), obs); }
}
