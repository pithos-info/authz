...
Role-based access control and identity for the Pithos platform. Manages users, groups, roles, permissions, and API keys across Cloud SQL and Postgres backends. Also owns the **Account** abstraction — the unified tenant object that supports both consumer (personal) and SaaS (team) applications built on Pithos.
...
- All entity tables have a `deleted BOOLEAN` column. `store.softDelete(ctx, id)` sets `deleted = true`. This includes `Account`.
...
The `account` table uses a `CHECK` constraint rather than a nullable discriminator column to enforce that exactly one of `ownerId` (PERSONAL) or `enterpriseId` (TEAM) is set. This is DB-level enforcement that cannot be bypassed by application code.
